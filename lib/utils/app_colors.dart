import 'package:flutter/material.dart';

class AppColors{
  static const Color blackColor=Color(0xFF121312);
  static const Color babyBlackColor=Color(0xFF282A28);
  static const Color transparentBlackColor=Color(0xB5121312);
  static const Color fullyTransparentBlackColor=Color(0x33121312);
  static const Color whiteColor=Color(0xFFFFFFFF);
  static const Color yellowColor=Color(0xFFFFBB3B);
  static const Color greenColor=Color(0xFF57AA53);
  static const Color redColor=Color(0xFFE82626);
  static const Color greyColor=Color.fromARGB(255, 109, 109, 109);
  static const Color lightWhiteColor=Color(0x99ffffff);
  static const Color transparentColor=Colors.transparent;
}